...
{
  MASShortcut *oldValue = _shortcutValue;
    _shortcutValue = shortcutValue;
    [self resetToolTips];
    [self setNeedsDisplay:YES];

    if (self.shortcutValueChange) {
        self.shortcutValueChange(self, oldValue);
    }
}
...
- (void)drawRect:(CGRect)dirtyRect
{
    if (self.shortcutValue) {
      ...
      [self drawInRect:self.recording ? shortcutRect : self.bounds withTitle:title alignment:NSCenterTextAlignment state:self.isRecording ? NSOnState : NSOffState];
    }
    else {
        if (self.recording)
        {
...
- (BOOL)locationInShortcutRect:(CGPoint)location
{
    CGRect shortcutRect;
    [self getShortcutRect:&shortcutRect hintRect:NULL];
    return CGRectContainsPoint(self.bounds, [self convertPoint:location fromView:nil]);
}
